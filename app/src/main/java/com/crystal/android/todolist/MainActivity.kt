package com.crystal.android.todolist
...
import com.crystal.android.todolist.datas.Todo
import com.crystal.android.todolist.ui.theme.TodoListTheme
...
@Composable
    get() = if (isLight) colorResource(R.color.gray_blue) else Color.White

val Colors.secondaryColor: Color
@Composable
    get() = if (isLight) Color.White else colorResource(R.color.gray_dark_blue)
...
fun TodoItem(item: Todo, onChange: (todo: Todo) -> Unit, onDelete: () -> Unit) {
    Card(modifier = Modifier
        ...
        Row(modifier = Modifier.padding(16.dp)) {
            if (item.onEdit) {
                TextField(
                    value = item.text, onValueChange = { onChange(item.copy(text = it)) },
                    modifier = Modifier
                        .weight(1f)
                        .padding(end = 8.dp)
                )
                ...
                Checkbox(
                    checked = item.done, onCheckedChange = { onChange(item.copy(done = it)) },
                    modifier = Modifier.align(Alignment.CenterVertically),
